...
list_of_passwords = list(map(str, passwords_string.split('\n')))
password_dictionary = []


def password_policy_1():
    counter_correct_psw = 0


    for item in list_of_passwords:
        index_semicolon = (item.find(":"))
        policy = item[:index_semicolon]
        password = item[index_semicolon + 1:]
        password_dictionary.append({'policy': policy, 'password': password})

    for item in password_dictionary:
        letter = (item['policy'][-1])
        letter_count = int((item['password']).count(letter))
        min_max_count = item['policy'].split('-')
        min_max_count[1] = min_max_count[1][:-2]
        if letter_count >= int(min_max_count[0]) and letter_count <= int(min_max_count[1]):
            counter_correct_psw = counter_correct_psw + 1

    print(f"There are {counter_correct_psw} correct passwords")
...
def password_policy_2():
    ...
    for item in password_dictionary:
        letter = (item['policy'][-1])
        min_max_count = item['policy'].split('-')
        first_position = int(min_max_count[0])
        second_position = int(min_max_count[1][:-2])

        condition_one = letter == item['password'][first_position] and not letter == item['password'][second_position]
        condition_two = letter == item['password'][second_position] and not letter == item['password'][first_position]
